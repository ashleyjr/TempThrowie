import Part
import math
from FreeCAD import Base

# Run in Freecad
# exec(open(<path_to_file>,'r').read())

# Parameters
base_length       = 37.5
base_width        = 40
base_depth        = 20
stantion_diameter = 5.4
probe_radius      = 0.65
probe_num         = 2
probe_positions   = [
    [3.6,   26.9],
    [3.6,   24.4],
    [4.2,   15.2],
    [6.7,   26.9],
    [9.7,   26.9],
    [12.7,  26.9],
    [14.8,  17.9],
    [14.8,  14.9],
    [33.2,  25.1],
        ]
wedge_length = 10
wedge_width = 4
wedge_depth = 6
wedge_x = 22
wedge_y = 12
wedge_z = -wedge_depth

trench1_length = base_length
trench1_width = 16
trench1_depth = 13
trench1_x = 0
trench1_y = 13
trench1_z = base_depth-trench1_depth

trench2_length = base_length
trench2_width = 19
trench2_depth = 11
trench2_x = 0
trench2_y = 11.5
trench2_z = base_depth-trench2_depth

# Name
name = "jgg_top"

# Create new document
if len(App.listDocuments()) != 0:
    for doc in App.listDocuments():
        App.setActiveDocument(doc)
        App.closeDocument(doc)
App.newDocument(name)
App.setActiveDocument(name)
App.ActiveDocument=App.getDocument(name)
Gui.ActiveDocument=Gui.getDocument(name)

# The major base
base=Part.makeBox(base_length,base_width,base_depth)

# The wedge
wedge=Part.makeBox(wedge_length,wedge_width,wedge_depth)
wedge.translate(Base.Vector(wedge_x,wedge_y,wedge_z))

# Trench 1
trench1=Part.makeBox(trench1_length,trench1_width,trench1_depth)
trench1.translate(Base.Vector(trench1_x,trench1_y,trench1_z))

# Trench 2
trench2=Part.makeBox(trench2_length,trench2_width,trench2_depth)
trench2.translate(Base.Vector(trench2_x,trench2_y,trench2_z))

# Stantion cuts
stantion0=Part.makeCylinder(
    stantion_diameter,
    base_depth)
stantion1=stantion0.copy()
stantion1.translate(Base.Vector(base_length,0,0))
stantion2=stantion0.copy()
stantion2.translate(Base.Vector(base_length,base_width,0))
stantion3=stantion0.copy()
stantion3.translate(Base.Vector(0,base_width,0))
stantion4=stantion0.copy()
stantion4.translate(Base.Vector(base_length/2,0,0))
base=base.cut(stantion0)
base=base.cut(stantion1)
base=base.cut(stantion2)
base=base.cut(stantion3)
base=base.cut(stantion4)

# Probe cuts
probe=Part.makeCylinder(
    probe_radius,
    base_depth)

probes = []
for pos in probe_positions:
    x = probe.copy()
    x.translate(Base.Vector(pos[0],pos[1],0))
    base=base.cut(x)

base=base.cut(trench1)
base=base.cut(trench2)
Part.show(wedge)
Part.show(base)


